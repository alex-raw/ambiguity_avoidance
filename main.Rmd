...
and take account for lexical items with very specific contextually bound uses,
...
Exclusion of those observations slightly improved distributional properties of the models.
...
the model framework chosen was GAMLSS [@gamlss] that allow the use of smoothing spline and fitting the required inflated beta distribution.
The models were created with the R package *gamlss* [@stasinopoulos2017].

Predicting counts of *-s* directly with poisson or negative binomial regression leads to problematic model properties.

The function of the following models is mostly a first exploration of the above-mentioned measures and their influence on inflection across the verb-noun continuum.
